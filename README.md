...
(2024/12/25) 最新のバージョンは `1.3.0` です。このバージョンでは、次の機能が追加されました。
...
: 登録単語をテーブル形式で表示して編集と削除、または単語を追加できる python による外部ツールです (リンク先は別リポジトリ)。
...
機能名が長いですが、変換バッファの先頭に余計な文字が入っていて長々と入力したのにまったく変換されてなかったという悲しみから解放されます。
...
どれも処理自体はほとんど同じなのでコピペで実装できしまうんですが手抜きではありません。

本来の用途の名前入力ではあまり使いどころのなさそうな本機能ですが、住所入力などで力を発揮します。

たとえば「c1=丁目、c2=番、c3=号」のように単語登録をしているときに、「1丁目2番3号」のような入力は結構めんどいです。
```
1(確定)c12(確定)c23(確定)c3
```
のような入力になると思いますが、絶対に IME 使った方が速そうですね。
...
「削除」型は先頭の 1 文字をバッファだけでなく、入力欄からも削除しますので、上の例でこちらを使うと、
```
丁目番号
```
...
1024-5-2号棟
```
のようなケースで使うと、「1024-5-2」をまとめて追い出して、残りの「号棟」を変換できます。このタイプだけは数字のみを対象として動作します。

たいていは打ち直した方が速い気がしますが、そんな機能もあったよね、的に使っていただければと思います。
